...
@section('content')
<div class=" ml-2 relative overflow-hidden shadow-md bg-gray-900 p-2 col-span-4">
          <div>
            <table class="w-full text-sm text-left rtl:text-right text-gray-200  border-black">
             ...
             <tbody>
                ...
                <tr class="">
                   ...
                   <td scope="row" class="border text-center px-6 py-4 whitespace-nowrap text-white">{{$row->judul_slide}}</td>
                   <td scope="row" class="border text-center px-6 py-4 whitespace-nowrap text-white">{{$row->artikel_id}}</td>
                   <td class="px-6 py-4 border text-center text-white">
                   @if($row->status == 1)
                        Active
                    @else
                        Draft
                   ...
                   <td class=" px-2 py-4 text-center border">
                       <div class="flex items-center justify-center">
                           <!-- <a href="{{route('slide.edit', $row->id)}}" class="font-medium text-white-600 dark:text-white-500 hover:text-white hover:underline"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
         ...
         <nav class="flex items-center flex-column flex-wrap md:flex-row justify-center pt-2" aria-label="Table navigation">
            <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8 justify-center">
                ...
                <li>
                    <a href="#" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-gray-900 border border-gray-300 hover:bg-gray-700 hover:text-redish  dark:border-gray-700 dark:text-gray-400  ">1</a>
                </li>
                <li>
